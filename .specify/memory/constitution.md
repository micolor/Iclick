...
The IClick project strictly prohibits the use of AppKit for UI components. SwiftUI is the exclusive UI framework. AppKit may only be used for system-level integrations where SwiftUI equivalents are not available.
...
The IClick Constitution supersedes all other development practices and guidelines. Amendments to this constitution require explicit documentation of changes, approval from project maintainers, and a migration plan for existing code. All development activities must verify compliance with these principles. Complexity in implementations must be justified through documentation as required by the Complex Code Documentation Principle.
